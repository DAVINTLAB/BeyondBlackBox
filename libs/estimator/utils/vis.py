    ...
    def __init__(self, dataset_name):
        if dataset_name == 'coco':
            ...
            self.EDGES = [([15, 13], [255, 0, 0]),  # l_ankle -> l_knee
                          ...
                          ([1, 2],   [0, 255, 5]),  # l_eye -> r_eye (7)
                          ([1, 0],   [0, 255, 170]),  # l_eye -> nose (8)
                          ([0, 2],   [0, 255, 25]),  # nose -> r_eye (9)
                          ([2, 4],   [0, 17, 255]),  # r_eye -> r_ear 
                          ([9, 7],   [0, 220, 0]),  # l_wrist -> l_elbow (11)
                          ([7, 5],   [0, 220, 0]),  # l_elbow -> l_shoulder (12)
                          ([5, 6],   [125, 125, 155]), # l_shoulder -> r_shoulder (13)
                          ([6, 8],   [25, 0, 55]),  # r_shoulder -> r_elbow (14)
                          ([8, 10], [25, 0, 255])]  # r_elbow -> r_wrist (15)
...
def plot_poses(img, skeletons, config=plt_config('coco'), save_path=None, dataset_name='coco'):
    ...
    for i in range(len(config.EDGES)):
        if i not in [7,8,9,11,12,13,14,15]:
            continue
        for j in range(len(skeletons)):
